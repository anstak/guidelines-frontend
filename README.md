...
- Использование транслита с русского в именах файлов, CSS-селекторах и т.п. недопустимо. Правильно, например: `.b-header`, неправильно: `.b-shapka`.
- Комментариев кириллицей лучше избегать, особенно в CSS и JavaScript.
- Отступы делаются пробелами.
...
- Решение средствами CSS (CSS3) лучше решения с графикой
- Cложное решение с CSS-хаками хуже решения с простым и понятным JavaScript
- Как правило кроссбраузерность подрузамевается IE8+, если заказчик не против, то можно IE9+. Желательно чтобы клиент смирился с квадратными углами в IE8 и IE9, отстутствием теней и градиентов.
...
### Структура
- Файлы могут быть статичными, но если верстаемых страниц больше одной, скорее всего имеет смысл вынести общие элементы в отдельные файлы и собирать страницу с помощью любой серверной технологии (в нашем случае RoR).
- Использование тегов в соответствии с [семантикой](http://www.w3school.ru/blog/xhtml/explaining-semantic-mark-up.html)
- Использование тегов по их назначению вместо `<div>` и `<span>` там, где это возможно. Т.к. мы используем HTML5, то использование тегов `<header>`, `<footer>`, `<navbar>` - обязательно.
...
### Содержимое
- HTML-код должен проходить валидацию по выбранному `DOCTYPE` (HTML5)
...
- Пустые ссылки (включая те, на которых будут только JavaScript-обработчики) должны выглядеть так: `<a href="#">Кликни меня</a>`, неприемлимые варианты пустых ссылок: `href="javascript:;"`, `href="/"`, `href=""`.

### HTML5
- Используем все возможности HTML5 (как и CSS3), берем вектор на будущее, меньше поддержки старых браузеров.
...
- Цвета должны указываться в HEX-формате заглавными буквами в полном формате, например: `#F3F3F3`.
